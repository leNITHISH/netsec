...
# --- Paste standard DES tables here exactly as in sender.py ---
...
# --- Helpers ---
def h2b(hex_str, n_bits): return list(bin(int(hex_str, 16))[2:].zfill(n_bits))
def b2h(bits): return hex(int("".join(bits), 2))[2:].upper().zfill(len(bits)//4)
def permutate(bits, table): return [bits[i-1] for i in table]
def shift(bits, n): return bits[n:] + bits[:n]
def xor(a, b): return [str(int(x) ^ int(y)) for x, y in zip(a, b)]
def b2d(b): return int("".join(b), 2)
def d2b(d, size): return list(bin(d)[2:].zfill(size))
...
def feistel(bits, subkey):
    ...
    for i in range(8):
        ...
        col = b2d(block[1:5])
        res.extend(d2b(S_BOX[i][row][col], 4))
    return permutate(res, P)

def des_block(bits, keys):
    # Decrypt: Same as encrypt but keys reversed
    keys = keys[::-1] 
...
def reciever():
    ...
    print(f"received: \"{payload_hex}\"")
    
    k_hex = input("enter 16-char hex key: ")
    key_bits = h2b(k_hex, 64)
    keys = generate_keys(key_bits)
    
    bits = h2b(payload_hex, 64)
    plain_bits = des_block(bits, keys)
    
    # decode bits to string
    res_str = ""
    for i in range(0, 64, 8):
        char_bits = plain_bits[i:i+8]
        res_str += chr(b2d(char_bits))
        
    print(f"decrypted: \"{res_str}\"")
    conn.close(); reci.close()
